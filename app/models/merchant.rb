class Merchant < ApplicationRecord
    ...
    def Merchant.parse_contract_pdf(file_path)
        ...
        cleaned_lines.map do |str|
            ...
            if str.include?(":")
                ...
                if array.length == 2
                    ...
                    # Get the first element, strip it of leading and trailing whitespaces, make it all lowercase and replace mid-whitespace with underscores.
                    raw_key = array[0].strip.downcase.gsub(/[\/\s]/, "_")
                    # Remove all ’ and turn the key from a string into a symbol.
                    key = raw_key.gsub("’", "").to_sym
                    value = array[1].strip
                    if hash[key]
                        # If the 'key' exists add the word 'physical' by converting key from sym to string then convert the entire thing back into symbol.
        ...
        first_merchant_filter = first_merchant_array.filter {|string| string != ""}
        second_merchant_filter = second_merchant_array.filter {|string| string != ""}

        first_length = first_merchant_filter.length
        second_length = second_merchant_filter.length

        first_merchant = first_length > 2 ? first_name = first_merchant_filter[1].strip : second_name = ""
        second_merchant = second_length > 2 ? second_name = second_merchant_filter[1].strip : second_name = ""
        ...
        # Determine and_1 and and_2 formatting words.
        and_1, and_2 = second_merchant == "" ? ["and", ""] : ["", "and"]
        ...
        if lender == "VelocityCapitalGroup"
        ...
        # If the merchants name is the same as the dba, leave the dba empty.
        if hash[:merchants_legal_name].include?(hash[:d_b_a]) 
            hash[:d_b_a] = ""
            # Set the d_b_a_p (p = placeholder) "key" to empty.
            d_b_a_p= ""
        else
            # Set the d_b_a_p (p = placeholder) "key" to "d/b/a".
            d_b_a_p = "d/b/a"
        end

        # Add the result to the hash...
        hash[:d_b_a_p] = d_b_a_p
    ...
    def Merchant.parse_payment_history_pdf(file_path)
        ...
        deafult_fee_line = payment_array_lines.find {|line| line.include?('Default Fee')}
        ...
        bounced_fee_lines = payment_array_lines.filter{|line| line.include?('Bounce Fee')}
        ...
        default_date_line = payment_array_lines.find {|str| str.include? "Default Fee"}
